"""Script to run the arXiv scraper with specific warp drive related keywords."""
...
from data_collection.arxiv_scraper import ArXivScraper
...
def get_search_combinations():
    ...
    combinations.extend(core_terms + physics_terms + propulsion_terms + 
                      energy_terms + materials_terms + theory_terms)
    ...
    # Combine theory with materials
    combinations.extend([f"{theory} {material}" 
                       for theory, material in product(theory_terms, materials_terms)])
    ...
    specific_combinations = [
        ...
        "topological warp drive",
        "quantum metamaterial propulsion",
        "spacetime topology drive",
        "closed timelike curve propulsion"
    ]
...
def main():
    try:
        # Initialize scraper
        output_dir = os.path.join("data", "arxiv")
        ...
        for i in range(0, len(search_terms), batch_size):
            ...
            scraper.scrape_all_categories(
                keywords=batch,
                max_results=100,  # Papers per category per batch
        ...
        logger.error(f"Error during arXiv scraping: {str(e)}", exc_info=True)
        sys.exit(1)
